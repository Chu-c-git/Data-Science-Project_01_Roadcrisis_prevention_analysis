...
def process_soil_liquid(hex_5m_sample, soil_liquid, epsg_code=3826):
    ...
    hex_5m_rd_sample_s = gpd.sjoin(hex_5m_sample, soil_liquid, how='inner', predicate='intersects')
    ...
    hex_5m_rd_sample_s["soil_liquid_class"] = hex_5m_rd_sample_s["soil_liquid_class"].replace(soil_replace)
...
def calculate_pipeline_count(hexagon_df, pipeline_gdf, count_column='sp_count'):
    ...
    for idx, row in tqdm(hexagon_df_copy.iterrows(), total=len(hexagon_df_copy), desc=count_column):
        buffer_zone = row['geometry']
        hexagon_df_copy.at[idx, count_column] = pipeline_gdf.intersects(buffer_zone).sum()
...
def create_buffered_gdf(df, lon_col, lat_col, buffer_distance=5, geometry_col='geometry'):
    """
    ...
    # 創建緩衝區
    df.loc[:, geometry_col] = df.apply(lambda x: x.centroid.buffer(buffer_distance), axis=1)
...
def calculate_case_count(hexagon_df, case_gdf, count_column='sp_count'):
    ...
    # Spatial join
    hexagon_df_copy = gpd.sjoin(hexagon_df_copy, case_gdf, how="left", predicate="intersects")
    hexagon_df_copy = hexagon_df_copy.drop(columns=drop_col)

    # Counting
    hexagon_df_copy['index_right'] = hexagon_df_copy['index_right'].fillna(0).astype(int)
    count_result = hexagon_df_copy.groupby('id').size()
    hexagon_df_copy[count_column] = hexagon_df_copy['id'].map(count_result)

    # Fill 0 intersection
    hexagon_df_copy[count_column] = hexagon_df_copy[count_column].where(hexagon_df_copy['index_right'] != 0, 0)
...
def calculate_case_count_v2(hexagon_df, case_gdf, count_column='sp_count', groupby='id'):
    """
    計算每個 hexagon 中管線的數量並更新 DataFrame。
...
def calculate_case_during_period(
    ...
    keep_col = ["geometry"]
    drop_col = [col for col in incidents_in_window.columns.tolist() if col not in keep_col]
    
    # Spatial join
    hexagon_df_copy = gpd.sjoin(hexagon_df, incidents_in_window, how="left", predicate="intersects")
    hexagon_df_copy = hexagon_df_copy.drop(columns=drop_col)
    ...
    # Fill 0 intersection
    hexagon_df_copy.loc[:, count_column] = hexagon_df_copy[count_column].where(hexagon_df_copy['index_right'] != 0, 0)
    right_col = [col for col in hexagon_df_copy.columns if col.endswith('right')]
    hexagon_df_copy = hexagon_df_copy.drop(columns=right_col)
    hexagon_df_copy = hexagon_df_copy.rename(columns={"geometry_left": "geometry"})
    hexagon_df_copy = hexagon_df_copy.drop_duplicates(subset=["id"])
...
def calculate_case_during_period_boolean(
    ...
    hexagon_df_copy = hexagon_df_copy.rename(columns={"geometry_left": "geometry"})
    hexagon_df_copy = hexagon_df_copy.drop_duplicates(subset=["id"])
    hexagon_df_copy[count_column] = hexagon_df_copy[count_column].apply(lambda x: 1 if x > 0 else 0)
    hexagon_df_copy = gpd.GeoDataFrame(hexagon_df_copy, geometry="geometry")
...
def find_average_rainfall(time_data, time_start, time_end, column='daily_precipitation'):
...
def process_rainfall_data(time_data, time_start, time_pred, hex_5m_rd_sample_cn, column='precipitation'):
...
def process_sum_data(time_data, time_start, time_pred, hex_5m_rd_sample_cn, column='precipitation'):
    # Create mask based on time range
...
def process_mean_data(time_data, time_start, time_pred, hex_5m_rd_sample_cn, column='precipitation'):
    # Create mask based on time range
    mask = (time_data["date"] >= time_start) & (time_data["date"] < time_pred)
...
def data_to_linestring(pos_list_text):
    ...
    # 將座標列表轉換成符合需求的格式
    input_str = [(coordinates[i], coordinates[i + 1], coordinates[i + 2]) for i in range(0, len(coordinates), 3)]
